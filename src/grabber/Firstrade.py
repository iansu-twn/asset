...
class Firstrade(Asset):
    ...
    def login(self):
        ft_ss = account.FTSession(username=self.uid, password=self.pwd, pin = self.code)
        need_code = ft_ss.login()
        if need_code:
            code = input("Please enter the pin sent to your email/phone: ")
            ft_ss.login_two(code)
        self.ft_accounts = account.FTAccountData(ft_ss)
    ...
    def info(self):
        res = self.ft_accounts.all_accounts
        val = res.get("grand_total")
